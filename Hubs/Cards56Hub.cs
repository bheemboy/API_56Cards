...
namespace API_56Cards.Hubs
{
    ...
    public class Cards56Hub : Hub<ICards56HubEvents>, ICards56Hub
    {
        ...
        public async Task RegisterPlayer(string playerName, string lang, bool watchOnly)
        {
            ...
            {
                await Clients.Caller.OnError((e is Card56Exception exception) ? exception.ErrorData!.ErrorCode: 0,
                    Cards56HubMethod.RegisterPlayer, 
                    e.Message, 
                    (e is Card56Exception exception1) ? exception1.ErrorData: null);
            }
        }

        private void GetPlayersCulture()
        {
            if (Context.Items.TryGetValue("Lang", out var langValue) && langValue is string lang)
                Thread.CurrentThread.CurrentCulture = CultureInfo.CreateSpecificCulture(lang);
        }
        ...
        public async Task PassBid()
        {
            ...
            {
                await Clients.Caller.OnError((e is Card56Exception exception1) ? exception1.ErrorData!.ErrorCode: 0,
                    Cards56HubMethod.PassBid, 
                    e.Message, 
                    (e is Card56Exception exception) ?exception.ErrorData: null);
            }
        }
